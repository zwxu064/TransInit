...
def init_spixel_grid(args, b_train=True, batch_size=None):
    if b_train:
...
def update_spixl_map (spixl_map_idx_in, assig_map_in):
    ...
    if False:  # Zhiwei avoid when multi-pixels have the same assig_max say 0.5, 0.5
        ...
        assig_argmax = torch.argmax(assig_map, dim=1, keepdim=True)
        assignment_ = assig_map_in.new_zeros(b, c, h, w, dtype=torch.float32).scatter_(1, assig_argmax, 1)
