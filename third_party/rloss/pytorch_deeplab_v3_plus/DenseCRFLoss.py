...
class DenseCRFLossFunction(Function):
    ...
    def forward(ctx, images, segmentations, sigma_rgb, sigma_xy, ROIs):
        ...
        print('=====>', AS.min(), AS.max(), segmentations.min(), segmentations.max())
        ...
        # plt.figure()
        # plt.subplot(131)
        # plt.imshow(Image.fromarray(np.transpose(np.reshape(images.astype(np.uint8), (ctx.N, 3, ctx.H, ctx.W)), (0,2,3,1))[0, :, :, :]))
        # plt.subplot(132)
        # plt.imshow(Image.fromarray(np.reshape(AS.astype(np.uint8), (ctx.N, ctx.K, ctx.H, ctx.W))[0,10,:,:]))
        # plt.subplot(133)
        # pred = np.argmax(myseg.detach().cpu().numpy(), axis=1)
        ...
        # scio.savemat('as_0.mat',
        #              {'image': np.reshape(images.astype(np.uint8), (ctx.N, 3, ctx.H, ctx.W)).transpose(2,3,1,0),
        #               'as': np.reshape(AS.astype(np.uint8), (ctx.N, ctx.K, ctx.H, ctx.W)).transpose(2,3,1,0)})
