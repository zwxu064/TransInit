...
class COCOSegmentation(Dataset):
    ...
    def __init__(self,
                 ...
                 year='2017'):
        super().__init__()
        base_dir = Path.db_root_dir('coco', data_root=args.data_root, server=server)
        ...
        self.args = args
        print('==> Number of {} images: {}'.format(self.split, len(self.ids)))

    def __getitem__(self, index):
        _img, _target, _name = self._make_img_gt_point_pair(index)
        sample = {'image': _img, 'label': _target,
                  'name': _name, 'size': (_img.height, _img.width)}
    ...
    def _preprocess(self, ids, ids_file):
        print("Preprocessing m[ask, this will take a while. " + \
