...
class ADE20KSegmentation(BaseDataset):
    ...
    def __init__(self, root=os.path.expanduser('./datasets/ADE20K'), split='train',
        ...
        root = Path.db_root_dir('ade20k', server='data61')  # Zhiwei
    ...
    def __getitem__(self, index):
        ...
        if self.mode == 'test':
            ...
            sample = {'image': img, 'name': os.path.basename(self.images[index])}
            return sample
            # return img, None, os.path.basename(self.images[index])
        ...
        sample = {'image': img, 'label': mask, 'name': os.path.basename(self.images[index])}
        return sample
