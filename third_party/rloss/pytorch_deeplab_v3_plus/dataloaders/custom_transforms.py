...
class Normalize(object):
    ...
    def __call__(self, sample):
        img = sample['image']
        mask = sample['label'] if ('label' in sample) else None
        edge = sample['edge'] if ('edge' in sample) else None
        img = np.array(img).astype(np.float32)
        mask = np.array(mask).astype(np.float32) if (mask is not None) else None
...
class ColorJitter(object):
    def __call__(self, sample):
        img = sample['image']
        sample['image'] = trans.ColorJitter(brightness=0.5, contrast=0.5, saturation=0.5)(img)
...
class AutoAdjustSize(object):
    ...
    def __call__(self, sample):
        ...
        padw = max(0, ow - w)
        img = ImageOps.expand(img, border=(0, 0, padw, padh), fill=0)
        mask = ImageOps.expand(mask, border=(0, 0, padw, padh), fill=self.fill) if (mask is not None) else None
        padding_mask = ImageOps.expand(padding_mask, border=(0, 0, padw, padh), fill=0) if (padding_mask is not None) else None
        edge = ImageOps.expand(edge, border=(0, 0, padw, padh), fill=0) if (edge is not None) else None
...
class DeNorm(object):
    ...
    def __call__(self, sample):
        return denormalizeimage(sample, self.mean, self.std)